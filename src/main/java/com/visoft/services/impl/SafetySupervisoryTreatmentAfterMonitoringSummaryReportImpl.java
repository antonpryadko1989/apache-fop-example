...
import static com.visoft.services.XSSFCellStyleService.getSafetySupervisoryTreatmentAfterMonitoringSummaryReportStyles;
...
import static com.visoft.utils.excelReportUtils.SummaryReportServiceConst.SAFETY_SUPERVISORY_TREATMENT_AFTER_MONITORING_SUMMARY_REPORT_ROW_VALUES;
import static com.visoft.utils.excelReportUtils.SummaryReportServiceConst.SAFETY_SUPERVISORY_TREATMENT_AFTER_MONITORING_SUMMARY_REPORT_TITLE;
...
public class SafetySupervisoryTreatmentAfterMonitoringSummaryReportImpl implements XLSXSummaryReportBuilder {
    ...
    public StreamingResponseBody buildXLSX(SummaryReportDTO summaryReport) throws IOException {
        ...
        XLSXObject xlsxObject = new XLSXObject(0, workbook.createSheet(SAFETY_SUPERVISORY_TREATMENT_AFTER_MONITORING_SUMMARY_REPORT_SHEET_NAME));
        setSummaryReportColumnWidths(xlsxObject.getSheet(), summaryReport.getName());
        setSummaryReportSheetZomm(xlsxObject, 50);
        XSSFCellStyle[] styles = getSafetySupervisoryTreatmentAfterMonitoringSummaryReportStyles(xlsxObject);
        addEmptyStringWithGivenHeight(xlsxObject, 24);
        addSummaryReportHeader(
                xlsxObject, 8, 11, styles[1],
                SAFETY_SUPERVISORY_TREATMENT_AFTER_MONITORING_SUMMARY_REPORT_HEADER_NAME, 55
        );
        addEmptyStringWithGivenHeight(xlsxObject, 24);
        ...
        addEmptyStringWithGivenHeight(xlsxObject, 24);
        addSummaryReportRow(xlsxObject, SAFETY_SUPERVISORY_TREATMENT_AFTER_MONITORING_SUMMARY_REPORT_TITLE, styles[0], true);
            ...
            for (int i = 0; i < summaryReport.getValues().size();i++){
                addSummaryReportRow(
                        ...
                        convertMapToSummaryReportRow(
                                ...
                                SAFETY_SUPERVISORY_TREATMENT_AFTER_MONITORING_SUMMARY_REPORT_ROW_VALUES,
                                i + 1),
                        styles[10], false);
            }
        }
